...
class BaseDocumentTestCase:
    ...
    def test_prepare_ignore_base_field(self):
        @registry.register_document
        class CarDocumentBaseField(Document):
            position = GeoPoint()
    ...
    def parse_body(self, call_args):
        # opensearch-py 2.x passes the body positionally, 3.x by keyword.
        body = call_args[0][0] if call_args[0] else call_args[1]["body"]
    ...
    def test_should_index_object_is_called(self, _mock_bulk):
        ...
        car2 = Car()
        car3 = Car()
        should_index_object = "django_opensearch_models.documents.Document.should_index_object"
...
class RealTimeDocumentTestCase(BaseDocumentTestCase, TestCase):
    TARGET_PROCESSOR = "django_opensearch_models.signals.RealTimeSignalProcessor"


class CeleryDocumentTestCase(BaseDocumentTestCase, TestCase):
