...
class Command(BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.os_conn = connections.get_connection()
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--create", action="store_const", dest="action", const="create", help="Create the indices in OpenSearch"
        )
        parser.add_argument(
            "--populate",
            action="store_const",
        ...
        parser.add_argument(
            "--delete", action="store_const", dest="action", const="delete", help="Delete the indices in OpenSearch"
        )
        parser.add_argument(
            "--rebuild",
            action="store_const",
        ...
        parser.add_argument("-f", action="store_true", dest="force", help="Force operations without asking")
        parser.add_argument(
            "--parallel", action="store_true", dest="parallel", help="Run populate/rebuild update multi threaded"
        )
        parser.add_argument(
            "--no-parallel", action="store_false", dest="parallel", help="Run populate/rebuild update single threaded"
        )
        parser.add_argument("--use-alias", action="store_true", dest="use_alias", help="Use alias with indices")
        parser.add_argument(
    ...
    def _get_models(self, args):
        ...
        if args:
            models = []
            for arg in args:
                arg = arg.lower()  # ruff: ignore[redefined-loop-name]
                match_found = False

                for model in registry.get_models():
                    if (
                        model._meta.app_label == arg
                        or f"{model._meta.app_label.lower()}.{model._meta.model_name.lower()}" == arg
                    ):
                        models.append(model)
                        match_found = True

                if not match_found:
                    msg = f"No model or app named {arg}"
    ...
    def _create(self, models, aliases, options):
        for index in registry.get_indices(models):
            ...
            if not alias_exists:
                ...
                self.stdout.write(
                    f"'{index._name}' already exists as an alias. Run '--delete' with"
                    " '--use-alias' arg to delete indices pointed at the "
                    "alias to make index name available."
                )
    ...
    def _delete_alias_indices(self, alias):
        ...
        self.os_conn.indices.update_aliases(body={"actions": alias_delete_actions})
    ...
    def _delete(self, models, aliases, options):
        ...
        if not options["force"]:
            response = input("Are you sure you want to delete the '{}' indices? [y/N]: ".format(", ".join(index_names)))
            if response.lower() != "y":
        ...
        if options["use_alias"]:
            for index in index_names:
                ...
                elif self.os_conn.indices.exists(index=index):
                    self.stdout.write(
                        f"'{index}' refers to an index, not an alias. Run "
            ...
            for index in registry.get_indices(models):
                ...
                if not alias_exists:
                    ...
                    self.stdout.write(
                        f"'{index._name}' refers to an alias, not an index. Run "
                        "'--delete' with '--use-alias' arg to delete indices "
                        "pointed at the alias."
                    )
                    return False
    ...
    def _update_alias(self, alias, new_index, alias_exists, options):
        ...
        if not alias_exists and self.os_conn.indices.exists(index=alias):
        ...
        self.os_conn.indices.update_aliases(body={"actions": alias_actions})
    ...
    def _rebuild(self, models, aliases, options):
        ...
        if options["use_alias"]:
            alias_index_pairs = []
            index_suffix = "-" + timezone.now().strftime("%Y%m%d%H%M%S%f")
            for index in registry.get_indices(models):
    ...
    def handle(self, *args, **options):
        if not options["action"]:
            msg = "No action specified. Must be one of '--create','--populate', '--delete' or '--rebuild'."
            raise CommandError(msg)
        ...
        aliases = []
        for index in self.os_conn.indices.get_alias().values():
        ...
        if action == "create":
            ...
            msg = "Invalid action. Must be one of '--create','--populate', '--delete' or '--rebuild'."
            raise CommandError(msg)
